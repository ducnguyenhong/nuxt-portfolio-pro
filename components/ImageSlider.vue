<template>
    ...
    <div v-for="(img, index) in images" :key="index" class="flex flex-1 w-full">
      <button type="button" @click="onShowImage(index)" class="w-full">
        <NuxtImg
          :src="img"
          :alt="IMAGE_ALT"
          class="w-full h-48 md:h-36 rounded-lg object-cover border border-[#e6e6e6]"
        />
      </button>
    </div>
  </div>
  ...
  <UModal v-model="isOpen" :ui="{ width: 'w-full sm:max-w-6xl' }">
    <div class="p-5">
      <NuxtImg :src="currentImage" :alt="IMAGE_ALT" class="w-full h-auto rounded-md object-cover" />
    </div>
  </UModal>
</template>
